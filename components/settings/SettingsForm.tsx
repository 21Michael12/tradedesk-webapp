...
export default function SettingsForm({ initial }: SettingsFormProps) {
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await saveUserSettings({ default_symbol: symbol, default_commission: c })
  ...
  return (
    <form onSubmit={submit} className="bg-surface-container rounded-lg border border-outline-variant p-5 flex flex-col gap-5">
      <header className="flex items-center justify-between">
        <h3 className="font-title-md text-title-md text-on-surface">העדפות מסחר</h3>
        {success && (
          <span className="text-success font-label-caps text-label-caps">נשמר ✓</span>
        )}
      </header>
